// Interactive wrapper that lets a single sidebar walk through all 5 states.

function InteractivePopup({ initial = 'login' }) {
  const [state, setState] = React.useState(initial); // login | main | processing | preview | success
  ...
  return (
    <SidebarFrame>
      ...
      {/* Floating step rail — for prototype only */}
      <div style={{
        ...
        {['login','main','processing','preview','success'].map(s => (
          <button key={s}
            onClick={() => setState(s)}
            title={s}
            style={{
...
// Browser chrome with the Surat sidebar docked to the right edge (Chrome side-panel layout).
function BrowserContext({ children, sidebarWidth = 380 }) {
  const W = 1140;
  const H = 720;
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Side-panel toggle (active because the panel is open) */}
        <div style={{
      ...
      {/* Below-chrome row: page on left, sidebar on right */}
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        ...
        {/* Docked sidebar — Surat side panel */}
        <div style={{
          width: sidebarWidth,
          flexShrink: 0,
...
Object.assign(window, { InteractivePopup, BrowserContext });
