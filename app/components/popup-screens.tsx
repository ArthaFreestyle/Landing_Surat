...
import { TopBar } from "./popup-shell";

export function ScreenLogin({ onContinue }: { onContinue?: () => void }) {
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "28px 24px 20px" }}>
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
          ...
          <span className="sx-serif" style={{ fontSize: 19, fontWeight: 500, color: SURAT.ink, letterSpacing: "-0.025em", lineHeight: 1 }}>Surat</span>
        </div>
      </div>
...
export function ScreenMain({ dragOver = false, onUpload }: { dragOver?: boolean; onUpload?: () => void }) {
    ...
    <>
      ...
      <div className="sx-scroll" style={{ flex: 1, padding: "16px 16px 0" }}>
        <div
          style={{
            ...
            background: SURAT.successSoft, marginBottom: 14,
            fontSize: 11.5, color: "#3d5a2e", lineHeight: 1.3,
          }}
        >
        ...
        <div className={"sx-drop " + (dragOver ? "over" : "")} style={{ padding: "28px 18px", textAlign: "center" }}>
          <div
            style={{
      ...
      >
        ...
        <button
          ...
          style={{ padding: "6px 11px", borderRadius: 8, fontSize: 12, background: SURAT.accentSoft, color: SURAT.accentInk, fontWeight: 500 }}
        >
          Upgrade
        </button>
      </div>
...
export function ScreenProcessing({ stage = 1 }: { stage?: number }) {
  const stages = [
  ...
  return (
    <>
      ...
      <div style={{ flex: 1, padding: "20px 20px 0", display: "flex", flexDirection: "column" }}>
        <div
          style={{
            position: "relative", height: 138, borderRadius: 10,
        ...
        >
          <div
            style={{
              ...
              fontFamily: SURAT.mono, fontSize: 9, color: "rgba(0,0,0,0.5)",
            }}
          >
...
export function ScreenPreview({ onFill, onBack }: { onFill?: () => void; onBack?: () => void }) {
  ...
  const toggle = (i: number) => setFields(fields.map((f, idx) => idx === i ? { ...f, on: !f.on } : f));
...
export function ScreenSuccess({ onDone, secondsLeft = 8 }: { onDone?: () => void; secondsLeft?: number }) {
  return (
    <>
      ...
      <div style={{ flex: 1, padding: "24px 20px 0", display: "flex", flexDirection: "column" }}>
        ...
        <div
          style={{
            ...
            background: SURAT.card, border: `1px solid ${SURAT.border}`, overflow: "hidden",
          }}
        >
          {[
        ...
        <div
          style={{
            ...
            background: SURAT.ink, color: "#f5efe0",
            display: "flex", alignItems: "center", gap: 10,
          }}
        >
      ...
      <div style={{ padding: "10px 16px 12px", borderTop: `1px solid ${SURAT.border}`, display: "flex", gap: 8 }}>
        <button className="sx-btn sx-btn-secondary" style={{ flex: 1 }}>Scan lagi</button>
        <button className="sx-btn sx-btn-primary" style={{ flex: 1 }} onClick={onDone}>Selesai</button>
      </div>
    </>
  );
}
